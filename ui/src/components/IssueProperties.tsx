...
export function IssueProperties({
  ...
  const runExecutionButton = (stageType: "review" | "approval") => (
    <PropertyRow label="">
      ...
      >
        {stageType === "review" ? t("issueProperties.runReviewNow") : t("issueProperties.runApprovalNow")}
      </button>
    </PropertyRow>
  );
  const currentExecutionLabel = (() => {
    ...
    if (issue.executionState.status === "changes_requested") {
      const base = isReview ? t("issueProperties.reviewRequestedChanges") : t("issueProperties.approvalRequestedChanges");
      return participantLabel ? `${base} ${t("issueProperties.by", { name: participantLabel })}` : base;
    }
    const base = isReview ? t("issueProperties.reviewPending") : t("issueProperties.approvalPending");
    return participantLabel ? `${base} ${t("issueProperties.with", { name: participantLabel })}` : base;
  })();
  ...
  const labelsExtra = (issue.labelIds ?? []).length > 0 ? (
    <button
      ...
      aria-label={t("issueProperties.addLabel")}
      title={t("issueProperties.addLabel")}
    >
  ...
  const labelsContent = (
    <>
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        ...
        >
          <Plus className="h-3 w-3" />
          {createLabel.isPending ? t("common.creating") : t("issueProperties.createLabel")}
        </button>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
          ...
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
            {creatorUserLabel ? t("issueProperties.assignTo", { name: creatorUserLabel }) : t("issueProperties.assignToRequester")}
          </button>
        )}
        {sortedAgents
          .filter((a) => {
  ...
  const parentTrigger = issue.parentId ? (
    ...
    <span className="text-sm text-muted-foreground">{t("issueProperties.noParent")}</span>
  );
  ...
  const parentContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("issueProperties.searchIssues")}
        value={parentSearch}
  ...
  const blockedByTrigger = blockedByIds.length > 0 ? (
    ...
    <span className="text-sm text-muted-foreground">{t("issueProperties.noBlockers")}</span>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <PropertyPicker
          inline={inline}
          label={t("issueProperties.blockedBy")}
          open={blockedByOpen}
          onOpenChange={(open) => {
        ...
        <PropertyRow label={t("issueProperties.blocking")}>
        ...
        <PropertyRow label={t("issueProperties.subIssues")}>
