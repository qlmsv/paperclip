    ...
    <>
      ...
      {collectingDownvoteReason ? (
        <div className="mt-2 rounded-md border border-border/60 bg-accent/20 p-3">
          <div className="mb-2 text-sm font-medium">{t("feedback.whatBetter")}</div>
          <Textarea
          ...
          <div className="mt-3 flex items-center justify-end gap-2">
            ...
            >
              {isSaving ? t("feedback.saving") : t("feedback.saveNote")}
            </Button>
          </div>
        </div>
      ) : null}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("feedback.savePreferenceTitle")}</DialogTitle>
            <DialogDescription>
              {t("feedback.savePreferenceDesc")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>
              {t("feedback.voteSavedLocally")}
            </p>
            <p>
              <Trans
                i18nKey="feedback.alwaysAllowDesc"
                components={{ 1: <span className="font-medium text-foreground" />, 3: <span className="font-medium text-foreground" /> }}
              />
            </p>
            <p>
              {t("feedback.changeLater")}
            </p>
            {termsUrl ? (
              <a
                href={termsUrl}
          ...
          <DialogFooter>
            ...
            >
              {isSaving ? t("feedback.saving") : t("feedback.dontAllow")}
            </Button>
            <Button
              type="button"
              disabled={!pendingVote || isSaving}
            ...
            >
              {isSaving ? t("feedback.saving") : t("feedback.alwaysAllow")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
