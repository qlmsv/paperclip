...
export function ReportsToPicker({
  ...
  const resolvedDisabledEmptyLabel = disabledEmptyLabel ?? t("reportsTo.reportsToCEO");
  const resolvedChooseLabel = chooseLabel ?? t("reportsTo.reportsToChoose");
        ...
        >
          {unknownManager ? (
            <>
              ...
              <span className="min-w-0 truncate text-muted-foreground">{t("reportsTo.unknownManager")}</span>
            </>
          ) : current ? (
            <>
              ...
              >
                {t("reportsTo.reportsToName", { name: current.name })}{terminatedManager ? t("reportsTo.terminated") : ""}
              </span>
            </>
          ) : (
            <>
